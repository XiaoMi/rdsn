...
public:
    ...
    // shared_tracer can record such case:
    // a_request-->start-->stageA -
    //                            |-->stageB-->end
    // b_request-->start-->stageA -
    // a_request and b_request could share same shared_tracer from stageB
    std::shared_ptr<latency_tracer> shared_tracer;

public:
    latency_tracer(uint64_t id, const std::string &start_name, const std::string &type)
    ...
    void set_shared_tracer(std::shared_ptr<latency_tracer> tracer) { shared_tracer = tracer; }

    uint64_t get_total_time_used()
    {
        if (shared_tracer != nullptr) {
            return shared_tracer->trace_points.back().ts - start_time;
        }

        return trace_points.back().ts - start_time;
    }
    ...
    std::string to_string() const
    {
        ...
        std::string trace_to_sub_trace =
            fmt::format("\n\tTRACER[{}]:from_previous={}, from_start={}, ts={}, name={}",
                        shared_tracer->id,
                        shared_tracer->start_time - previous_time,
                        shared_tracer->start_time - start_time,
                        shared_tracer->start_time,
                        shared_tracer->start_name);

        std::string sub_trace = shared_tracer->to_string();
