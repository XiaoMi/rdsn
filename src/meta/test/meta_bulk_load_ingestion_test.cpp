...
public:
    ...
    int32_t get_disk_count(const std::string &disk_tag)
    {
        if (_context.disk_count.find(disk_tag) == _context.disk_count.end()) {
            return -1;
        }
...
public:
    ...
    void mock_partition(const int32_t pidx,
    ...
    void remove_partition(const int32_t pidx) { _context->remove_partition(gpid(APP_ID, pidx)); }

    bool is_partition_ingesting(const int32_t pidx) const
    {
        return _context->_running_partitions.find(gpid(APP_ID, pidx)) !=
               _context->_running_partitions.end();
    }

    int32_t get_app_running_count() const { return _context->get_app_running_count(APP_ID); }
    ...
    int32_t get_disk_running_count(const rpc_address &node, const std::string &disk_tag)
    {
        ...
        if (node_cc.disk_count.find(disk_tag) == node_cc.disk_count.end()) {
            return 0;
        }
        return node_cc.disk_count[disk_tag];
    }

    bool validate_count(const rpc_address &node,
                        const int32_t expected_node_count,
                        const int32_t expected_disk1_count,
                        const int32_t expected_disk2_count)
    {
...
public:
    ...
    const int32_t APP_ID = 1;
    const int32_t PARTITION_COUNT = 4;
    const int32_t MAX_NODE_COUNT = 2;
    const int32_t MIN_DISK_COUNT = 2;
