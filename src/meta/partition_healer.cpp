...
/// partition healer extensions for node_state
...
//// end of partition healer extensions for node state

partition_healer::partition_healer(meta_service *svc)
{
    if (svc != nullptr) {
...
pc_status
partition_healer::cure(meta_view view, const dsn::gpid &gpid, configuration_proposal_action &action)
{
...
bool partition_healer::collect_replica(meta_view view,
...
bool partition_healer::construct_replica(meta_view view, const gpid &pid, int max_replica_count)
{
...
void partition_healer::register_ctrl_commands()
{
...
void partition_healer::unregister_ctrl_commands()
{
    UNREGISTER_VALID_HANDLER(_ctrl_assign_delay_ms);
    UNREGISTER_VALID_HANDLER(_ctrl_assign_secondary_black_list);
}

void partition_healer::get_ddd_partitions(gpid pid, std::vector<ddd_partition_info> &partitions)
{
...
void partition_healer::clear_ddd_partitions()
{
    zauto_lock l(_ddd_partitions_lock);
    _ddd_partitions.clear();
}

void partition_healer::set_ddd_partition(ddd_partition_info &&partition)
{
    zauto_lock l(_ddd_partitions_lock);
    _ddd_partitions[partition.config.pid] = std::move(partition);
}

void partition_healer::finish_cure_proposal(meta_view &view,
                                            const dsn::gpid &gpid,
...
bool partition_healer::from_proposals(meta_view &view,
                                      const dsn::gpid &gpid,
...
pc_status partition_healer::on_missing_primary(meta_view &view, const dsn::gpid &gpid)
{
    const partition_configuration &pc = *get_config(*(view.apps), gpid);
...
pc_status partition_healer::on_missing_secondary(meta_view &view, const dsn::gpid &gpid)
{
...
pc_status partition_healer::on_redundant_secondary(meta_view &view, const dsn::gpid &gpid)
{
...
std::string partition_healer::ctrl_assign_delay_ms(const std::vector<std::string> &args)
{
...
std::string partition_healer::ctrl_assign_secondary_black_list(const std::vector<std::string> &args)
{
