...
TEST(ip_to_hostname, locolhost)
{
    ...
    std::string success_ip_list = "127.0.0.1,127.0.0.1,127.0.0.1";
    ...
    std::string expected_hostname = "localhost";
    ...
    EXPECT_FALSE(replication_ddl_client::list_hostname_from_ip(
        "127.0.0.1,127.0.0.23323,111127.0.0.3", &hostnameResult));
    EXPECT_FALSE(replication_ddl_client::list_hostname_from_ip(
        "123.456.789.111,1123.456.789.111,0.0.3", &hostnameResult));

    // static bool list_hostname_from_ip_port(const char *ip_port_list,std::string
    // *ip_address_list);

    EXPECT_TRUE(replication_ddl_client::list_hostname_from_ip_port(success_ip_port_list.c_str(),
                                                                   &hostnameResult));
    EXPECT_STREQ(expected_hostname_port_list.c_str(), hostnameResult.c_str());

    EXPECT_FALSE(replication_ddl_client::list_hostname_from_ip_port(
