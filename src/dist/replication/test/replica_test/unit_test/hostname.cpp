...
TEST(ip_to_hostname, locolhost)
{
    ...
    // static bool hostname_from_ip(uint32_t ip, std::string* ip_address);
    EXPECT_EQ(1, replication_ddl_client::hostname_from_ip(htonl(rpc_example_success.ip()), &hostnameResult));
    EXPECT_EQ(0, replication_ddl_client::hostname_from_ip(htonl(rpc_example_failed.ip()), &hostnameResult));

    // static bool hostname_from_ip(const char *ip,std::string *ip_address);
    EXPECT_EQ(1, replication_ddl_client::hostname_from_ip("127.0.0.1", &hostnameResult));
    EXPECT_EQ(0, replication_ddl_client::hostname_from_ip("123.456.789.111", &hostnameResult));

    // static bool hostname_from_ip_port(const char *ip_port,std::string *ip_address);
    EXPECT_EQ(1, replication_ddl_client::hostname_from_ip_port("127.0.0.1:8080", &hostnameResult));
    EXPECT_EQ(0, replication_ddl_client::hostname_from_ip_port("123.456.789.111:8080", &hostnameResult));

    // static bool list_hostname_from_ip(const char *ip_port_list,std::string *ip_address_list);
    EXPECT_EQ(1,
              replication_ddl_client::list_hostname_from_ip("127.0.0.1,127.0.0.2,127.0.0.3",
                                                            &hostnameResult));
    EXPECT_EQ(1,
    ...
    EXPECT_EQ(1,
              replication_ddl_client::list_hostname_from_ip_port(
                  "127.0.0.1:23456,127.0.0.2:22233,127.0.0.3:12345", &hostnameResult));
    EXPECT_EQ(1,
              replication_ddl_client::list_hostname_from_ip_port(
